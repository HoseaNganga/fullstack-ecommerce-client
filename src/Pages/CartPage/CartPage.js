...
const CartPage = () => {
  ...
  const { setCartCount, cartData, setCartData, user } = useContext(DataContext);
  ...
  const updateQuantityAndSubtotal = async (productId, newQuantity) => {
    const itemToUpdate = cartData.cartList.find(
    ...
    const updatePromise = new Promise(async (resolve, reject) => {
      try {
        ...
        await modifyData(`/api/cart/${itemToUpdate._id}`, {
        ...
        setCartData((prevCartData) => ({
          ...prevCartData,
          cartList: prevCartData.cartList.map((item) =>
  ...
  const handleCartItemDelete = async (id) => {
    const deleteCartItemPromise = new Promise(async (resolve, reject) => {
      try {
        ...
        const res = await fetchDataFromApi(`/api/cart?userId=${user?.id}`);
        setCartData(res);

        // Ensure cartList exists before reducing
        if (res.cartList) {
          const newQuantities = res.cartList.reduce((acc, item) => {
    ...
    toast.promise(deleteCartItemPromise, {
      loading: "Cart Item is being deleted..",
      success: "Cart Item deleted Successfully",
      error: "Error..Cart Item Not deleted",
    });
  };
    ...
    const addOrderToDatabasePromise = new Promise(async (resolve, reject) => {
      try {
        ...
        if (response.success) {
          ...
          await deleteData(`/api/cart/clear/${clearId}`);
    ...
    const fetchCartData = async () => {
      try {
        if (user?.id) {
          ...
          const newQuantities = response.cartList.reduce((acc, item) => {
            acc[item.productId] = item.quantity || 1;
            return acc;
          }, {});
          setQuantities(newQuantities);
      ...
      <section className="section cartPage">
        <div className="container">
          ...
          <Typography variant="body1" gutterBottom>
            There are{" "}
            <b>{cartData?.cartList ? cartData?.cartList?.length : 0}</b>{" "}
